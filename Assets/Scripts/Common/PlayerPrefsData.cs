...
public class PlayerPrefsData : PlayerPrefs
{
    ...
    static PlayerPrefsData()
    {
        // 8 바이트로 하고, 변경해서 쓸것
        byte[] saltBytes = new byte[] { 45, 37, 8, 69, 5, 25, 34, 73 };
 
        // 길이 상관 없고, 키를 만들기 위한 용도로 씀
        string randomSeedForKey = "2e327725786d6b2ad8979841b5bb5c70";
 
        // 길이 상관 없고, aes에 쓸 key 와 iv 를 만들 용도
        string randomSeedForValue = "5b6fcb4aaba45a506eca0a42acae649e";
    ...
    private static bool HasSecurityValue(string key)
    {
        string hideKey = MakeHash(key + _saltForKey);
    ...
    public static new int GetInt(string key, int defaultValue)
    {
    ...
    public static new float GetFloat(string key, float defaultValue)
    {
        string originalValue = GetSecurityValue(key);
        if (true == string.IsNullOrEmpty(originalValue))
            return defaultValue;
    ...
    public static new bool HasKey(string key)
    {
        return HasSecurityValue(key);
    }
}
